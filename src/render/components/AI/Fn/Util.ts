...
export function killPort(this: BaseTask, ports: Array<string>) {
  return new Promise(async (resolve) => {
    ...
    for (const port of ports) {
      let res: any = execSync(`echo '${global.Server.Password}' | sudo -S lsof -nP -i:${port} | grep '(LISTEN)' | awk '{print $1,$2,$3,$9,$10}'`)
      res = res?.stdout ?? ''
    ...
    if (pids.size > 0) {
      ...
      try {
        execSync(`echo '${global.Server.Password}' | sudo -S kill -9 ${pidStr}`)
      } catch (e) {}
    }
    resolve(true)
  })
